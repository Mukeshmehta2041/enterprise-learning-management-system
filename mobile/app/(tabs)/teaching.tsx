...
export default function TeachingScreen() {
  ...
  const { data: courses, isLoading: coursesLoading, refetch: refetchCourses } = useInstructorCourses();
  ...
  const renderHeader = () => (
    <View className="px-4 pt-4">
      <View className="flex-row justify-between items-center mb-6">
        ...
        <TouchableOpacity testID="add-course-button" className="bg-indigo-600 w-10 h-10 rounded-full items-center justify-center shadow-md shadow-indigo-300"
  ...
  const renderCourseItem = ({ item }: { item: Course }) => (
    ...
    >
      ...
      <View className="flex-1">
        <AppText className="text-slate-900 font-bold mb-1" numberOfLines={1}>{item.title}</AppText>
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50" edges={['top']}>
      <FlatList
        ...
        ListEmptyComponent={() => !coursesLoading && (
          <View className="items-center justify-center py-20 px-10">
            ...
            <AppText className="text-slate-900 font-bold text-lg text-center mb-2">No Courses Yet</AppText>
            <AppText className="text-slate-500 text-center mb-6">Start sharing your knowledge by creating your first course.</AppText>
            <TouchableOpacity
