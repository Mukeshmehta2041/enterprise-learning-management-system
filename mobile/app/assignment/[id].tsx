  ...
  if (error || !assignment) {
    return (
      <View className="flex-1 items-center justify-center p-6 bg-background">
        ...
        <AppText variant="h3" className="mt-4">Failed to load assignment</AppText>
      ...
      <ScrollView className="flex-1 p-6">
        <View className="mb-6">
          <View className="flex-row items-center justify-between mb-2">
            <AppText variant="h1" className="flex-1 mr-2">{assignment.title}</AppText>
            <Badge
              label={assignment.status}
              variant={assignment.status === 'GRADED' ? 'success' : assignment.status === 'SUBMITTED' ? 'primary' : 'warning'}
            />
          </View>
        ...
        <Card className="mb-6">
          <AppText variant="caption" weight="bold" color="muted" className="mb-2">DESCRIPTION</AppText>
          <AppText variant="body" className="mb-4">{assignment.description}</AppText>
        ...
        {assignment.status === 'GRADED' && (
          <Card className="mb-6 border-green-200 bg-green-50">
            <AppText variant="caption" weight="bold" color="primary" className="mb-2">GRADE & FEEDBACK</AppText>
            <AppText variant="h2" color="primary">{assignment.grade}/100</AppText>
            <AppText variant="body" className="mt-2">{assignment.feedback || 'Excellent work!'}</AppText>
          </Card>
        )}

        {!isSubmitted ? (
          <View>
            <AppText variant="h3" className="mb-4">Your Submission</AppText>
            <Input
          ...
          <View>
            <AppText variant="h3" className="mb-4">My Submission</AppText>
